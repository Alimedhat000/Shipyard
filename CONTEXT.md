...
### Object Storage
S3-compatible blob storage for deployed artifacts. Abstracted behind an `uploadToObjectStorage()` interface using the AWS S3 SDK with `forcePathStyle: true`. MinIO for local dev; AWS S3 for production. No hardcoded S3-specific logic.
